# PyInstaller 전용: app_v1 등은 .pyd라 정적 분석이 안 되므로,
# 여기서 한 번 import 해 두면 분석 그래프에 의존성이 잡힘 (spec에 패키지 나열 불필요).
# requirements.txt와 맞춰 두고, 라이브러리 추가 시 여기만 수정하면 됨.

# Cython으로 같이 빌드되는 동일 프로젝트 모듈(.pyd) — 엔트리 .pyd 안의 import는 PyInstaller가 못 읽음
...
import member_app_shared  # noqa: F401
...
# 로그인(check_auth) — .pyd 안에서만 import 되면 번들에 안 잡힘
