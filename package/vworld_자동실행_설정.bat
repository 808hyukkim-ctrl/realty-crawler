...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$exe = Join-Path '%~dp0' '중개업소_신규개업수집기.exe';" ^
  ...
  "Register-ScheduledTask -TaskName '중개업소 신규개업 수집' -Action $a -Trigger $t -Settings $s -Description '신규 개업 중개업소 자동 수집 후 메일/텔레그램 전송' -Force | Out-Null;" ^
...
echo.
echo  [확인]  시작 메뉴에서 "작업 스케줄러"를 열면 '중개업소 신규개업 수집' 이 보입니다.
echo  [지금 시험]  아래에서 y 를 누르면 지금 한 번 돌려봅니다.
echo.
set "TESTRUN=n"
set /p TESTRUN=" 지금 한 번 시험 실행할까요? (y/n) : "
if /i "%TESTRUN%"=="y" (
    schtasks /run /tn "중개업소 신규개업 수집"
