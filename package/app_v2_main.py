...
class NaverWorker(QObject):
    ...
    def _display_row_from_naver(row: Any) -> tuple:
        if isinstance(row, (list, tuple)) and len(row) >= 9:
            no = str(row[3]) if len(row) > 3 else "-"
            ...
            floor = str(row[18]) if len(row) > 18 and row[18] else "-"
            hosu = str(row[11]) if len(row) > 11 and row[11] else "-"
            return (no or "-", region or "-", kind or "-", trade or "-", name or "-", floor or "-", hosu or "-")
    ...
    def run(self):
        try:
            ...
            mode = str(self.params.get("mode", "crawl")).strip().lower()

            if mode == "search":
                region_names = self.params.get("region_names", [])
                self.status.emit(f"네이버: 검색결과 조회 중... ({len(region_names)}개 동 대상)")
                candidates = crawler.search_articles_v1(
                ...
                self.search_ready.emit(candidates)
                if not candidates:
                    self.finished.emit("검색 결과가 없습니다.")
                else:
                    self.finished.emit(f"검색 완료: {len(candidates)}건 (체크 후 수집 시작)")
                return
            ...
            if mode == "crawl_selected":
                selected_articles = self.params.get("selected_articles", [])
                total = len(selected_articles)
                self.status.emit(f"네이버: 선택 매물 수집 중... ({total}건)")
                for i, article in enumerate(selected_articles, 1):
                    if self._cancel:
                        break
                    article_no = str((article or {}).get("article_no", "")).strip()
                    if not article_no:
                        continue
                    self.progress.emit(i, total)
                    self.status.emit(f"네이버: 선택 매물 수집 중... ({i}/{total}, {article_no})")
                    try:
                        row = crawler.extract_detail_v2(article_no)
                    except Exception:
                        row = None
                    if not row:
                        continue
            ...
            if not matched_rows:
                ...
                return
            self._export_naver_rows(matched_rows)
            prefix = "중단 저장 완료" if self._cancel else "저장 완료"
            self.finished.emit(f"{prefix}: {self._output_path()} ({len(matched_rows)}건)")
        except Exception as e:
            self.failed.emit(str(e))
        finally:
            self._phase = "idle"
    ...
    def _output_path(self) -> str:
        schedule_name = self.params.get("schedule_name")
        if schedule_name:
            fname = f"{schedule_name}_{self.params['result_count']}건_{self.params['timestamp']}.xlsx"
        else:
            fname = f"{self.params['site_name']}_{self.params['region_for_file']}_{self.params['result_count']}건_{self.params['timestamp']}.xlsx"
        return os.path.join(self.params["out_dir"], fname)

    def _export_naver_rows(self, matched_rows: List[Any]) -> None:
        schedule_name = self.params.get("schedule_name")
        if schedule_name:
            fname = f"{schedule_name}_{len(matched_rows)}건_{self.params['timestamp']}.xlsx"
        else:
            fname = f"{self.params['site_name']}_{self.params['region_for_file']}_{len(matched_rows)}건_{self.params['timestamp']}.xlsx"
        out_path = os.path.join(self.params["out_dir"], fname)
        self.params["result_count"] = len(matched_rows)
        import pandas as pd
        from openpyxl.cell.cell import ILLEGAL_CHARACTERS_RE

        df = pd.DataFrame(matched_rows)
        headers = [
            "시", "구", "동", "매물번호", "이미지URL", "등록/확인일", "집주인/확인", "종류", "거래방식", "매물명",
            "아파트동", "호수", "공급/계약/대지", "전용/연", "건면적", "전용률", "용적률", "건폐율", "해당층", "전체층", "방향",
    ...
    def _apply_excel_hyperlinks(path: str) -> None:
        """저장된 엑셀의 URL/링크 컬럼을 하이퍼링크 셀로 변환"""
        try:
            import openpyxl
        ...
        try:
            ...
            for col_idx in range(1, ws.max_column + 1):
                ...
                if ("url" in h) or ("link" in h) or ("링크" in header) or ("매물번호" in header):
...
class MainWindow(QMainWindow, ScheduleMixin):
    def __init__(self):
        ...
        self.nv_search_dialog: QDialog | None = None
        self.nv_search_table: QTableWidget | None = None
        self.nv_search_candidates: List[Dict[str, Any]] = []
        self.nv_search_groups: List[Dict[str, Any]] = []
        self.nv_last_params: Dict[str, Any] = {}
        self.nv_searching_dialog: QDialog | None = None
        self.nv_searching_status_label: QLabel | None = None
        self.nv_worker_mode: str = ""
        self._last_output_path: str | None = None
        self.setWindowTitle("전국부동산매물수집기_v2")
    ...
    def _build_daangn_tab(self) -> QWidget:
        ...
        self.cb_si = QComboBox()
        self.cb_si.addItems(self._region_si_list)
        self.cb_gun = QComboBox()
        self.cb_gu = QComboBox()
        ...
        row.addStretch(1)
        self.cb_si.currentTextChanged.connect(self._on_si_changed)
        self.cb_gun.currentTextChanged.connect(self._on_gun_changed)
        self._on_si_changed(self.cb_si.currentText())
        ...
        self.sl_month = DualSlider("월세(만)", 0, 3000, 10)
        self.sl_deposit = DualSlider("보증금(만)", 0, 50000, 1000)
        self.sl_buy = DualSlider("매매(만)", 0, 1000000, 5000)
        self.sl_borrow = DualSlider("전세(만)", 0, 1000000, 5000)
        self.sl_area = DualSlider("평수", 0, 200, 10)
        grid.addWidget(self.sl_month, 0, 0)
        grid.addWidget(self.sl_deposit, 0, 1)
        grid.addWidget(self.sl_buy, 1, 0)
        grid.addWidget(self.sl_borrow, 1, 1)
        grid.addWidget(self.sl_area, 2, 0)
        v.addWidget(price_box)
        detail_box = QGroupBox("상세 조건 (방수/층수/준공년도)")
        dgrid = QGridLayout(detail_box)
        self.dg_sl_rooms = DualSlider("방수", 0, 10, 1)
        self.dg_sl_floor = DualSlider("층수", 0, 50, 1)
        self.dg_sl_builtyear = DualSlider("준공년도", 1970, 2026, 1)
        ...
        # 건축물용도 필터 비활성화 (요청으로 UI/동작에서 제외)
        # self.dg_use_group = UseCheckGroup("건축물용도")
    ...
    def _build_naver_tab(self) -> QWidget:
        ...
        region_box = QGroupBox("지역")
        rr = QHBoxLayout(region_box)
        self.nv_cb_si = QComboBox()
        self.nv_cb_si.addItems(getattr(self, "_nv_rls_si_list", ["전체"]))
        self.nv_cb_gun = QComboBox()
        self.nv_cb_gu = QComboBox()
        ...
        rr.addStretch(1)
        self.nv_cb_si.currentTextChanged.connect(self._on_nv_si_changed)
        self.nv_cb_gun.currentTextChanged.connect(self._on_nv_gun_changed)
        self._on_nv_si_changed(self.nv_cb_si.currentText())
        # 네이버 탭 기본값: 서울시 - 전체 - 전체 (naver_rls 키)
        if self.nv_cb_si.findText("서울시") >= 0:
            self.nv_cb_si.setCurrentText("서울시")
        else:
            self.nv_cb_si.setCurrentText("전체")
        self._on_nv_si_changed(self.nv_cb_si.currentText())
        self.nv_cb_gun.setCurrentText("전체")
        self._on_nv_gun_changed(self.nv_cb_gun.currentText())
        self.nv_cb_gu.setCurrentText("전체")
        v.addWidget(region_box)
        ...
        price_box = QGroupBox("가격/면적/등록일")
        pg = QGridLayout(price_box)
        self.nv_sl_rent = DualSlider("월세(만)", 0, 3000, 10)
        self.nv_sl_warr = DualSlider("보증금(만)", 0, 50000, 1000)
        self.nv_sl_deal = DualSlider("매매가(만)", 0, 1000000, 1000)
        self.nv_sl_jeon = DualSlider("전세(만)", 0, 1000000, 1000)
        self.nv_sl_area = DualSlider("면적(㎡)", 0, 231, 1)
        pg.addWidget(self.nv_sl_rent, 0, 0)
        pg.addWidget(self.nv_sl_warr, 0, 1)
        ...
        v.addWidget(price_box)
        nv_detail_box = QGroupBox("상세 조건 (방수/층수/준공년도)")
        nv_dgrid = QGridLayout(nv_detail_box)
        self.nv_sl_rooms = DualSlider("방수", 0, 10, 1)
        self.nv_sl_floor = DualSlider("층수", 0, 50, 1)
        self.nv_sl_builtyear = DualSlider("준공년도", 1970, 2026, 1)
        ...
        # 건축물용도 필터 비활성화 (요청으로 UI/동작에서 제외)
        # self.nv_use_group = UseCheckGroup("건축물용도")
    ...
    def _on_si_changed(self, si: str):
        vals = self._region_si_gun_map.get(si, ["전체"]) if si != "전체" else ["전체"]
        self.cb_gun.blockSignals(True)
        self.cb_gun.clear()
        self.cb_gun.addItems(vals)
        self.cb_gun.blockSignals(False)
        self._on_gun_changed(self.cb_gun.currentText())

    def _on_gun_changed(self, gun: str):
        si = self.cb_si.currentText()
        if si == "전체" or gun == "전체":
            vals = ["전체"]
        else:
            vals = self._region_si_gun_gu_map.get(f"{si} {gun}", ["전체"])
        self.cb_gu.clear()
        self.cb_gu.addItems(vals)
    ...
    def _on_nv_si_changed(self, si: str):
        vals = self._nv_rls_si_gu_map.get(si, ["전체"]) if si != "전체" else ["전체"]
        self.nv_cb_gun.blockSignals(True)
        self.nv_cb_gun.clear()
        self.nv_cb_gun.addItems(vals)
        self.nv_cb_gun.blockSignals(False)
        self._on_nv_gun_changed(self.nv_cb_gun.currentText())

    def _on_nv_gun_changed(self, gun: str):
        si = self.nv_cb_si.currentText()
        if si == "전체" or gun == "전체":
            vals = ["전체"]
        else:
            vals = self._nv_rls_si_gu_dong_map.get(f"{si} {gun}", ["전체"])
        self.nv_cb_gu.clear()
        self.nv_cb_gu.addItems(vals)
    ...
    def _start_daangn(self):
        ...
        def _rng(values, full_min, full_max):
            lo, hi = values
            return None if (lo <= full_min and hi >= full_max) else (lo, hi)

        ts = datetime.now().strftime("%y%m%d_%H%M%S")
        out_dir = os.path.join(self.base_dir, "data")
        os.makedirs(out_dir, exist_ok=True)
        region_for_file = self._region_for_filename(
            self.cb_si.currentText() or "전체",
            self.cb_gun.currentText() or "전체",
            self.cb_gu.currentText() or "전체",
        )
        params = {
            "regions": self._selected_regions_for(
                self.cb_si.currentText(),
                self.cb_gun.currentText(),
                self.cb_gu.currentText(),
            ),
            ...
            # "detail_uses": self.dg_use_group.selected(),  # 건축물용도 필터 비활성화
            "site_name": "당근",
    ...
    def _start_naver(self):
        si = self.nv_cb_si.currentText()
        gun = self.nv_cb_gun.currentText()
        gu = self.nv_cb_gu.currentText()
        if si == "전체":
        ...
        # 상세 필터(방수/층수/준공년도)는 DetailFilters가 None(미적용) 또는 (lo, hi) 튜플을 받는다.
        ...
        params = {
            ...
            # "detail_uses": self.nv_use_group.selected(),  # 건축물용도 필터 비활성화
            "site_name": "네이버",
            "region_for_file": region_for_file,
            "timestamp": ts,
            "out_dir": out_dir,
        }
        self.nv_last_params = dict(params)
        self.nv_worker_mode = "search"
        self._prepare_worker_start()
        self.btn_start.setEnabled(False)
        self.btn_stop.setEnabled(True)
        self.progress_bar.setValue(0)
        self.progress_text.setText("네이버 검색 중")
        ...
        self.worker_thread.started.connect(self.worker.run)
        self.worker.row_ready.connect(self._append_log)
        self.worker.search_ready.connect(self._on_naver_search_ready)
        ...
        self.worker_thread.finished.connect(self._cleanup_worker)
        self.worker_thread.start()
        self._show_naver_searching_dialog("네이버 검색결과를 조회 중입니다...")

    def _on_naver_search_ready(self, candidates: List[Dict[str, Any]]):
        self.nv_search_candidates = list(candidates or [])
        self._close_naver_searching_dialog()
        grouped: Dict[str, Dict[str, Any]] = {}
        ordered_keys: List[str] = []
        for row in self.nv_search_candidates:
            name = str((row or {}).get("article_name", "")).strip()
            article_no = str((row or {}).get("article_no", "")).strip()
            if name:
                key = name
                display_name = name
            else:
                key = f"__NO_NAME__{article_no}"
                display_name = "(매물명없음)"
            if key not in grouped:
                grouped[key] = {"name": display_name, "items": []}
                ordered_keys.append(key)
            grouped[key]["items"].append(row)
        ordered_keys.sort(
            key=lambda k: (
                grouped[k].get("name", "") == "(매물명없음)",
                str(grouped[k].get("name", "")),
            )
        )
        self.nv_search_groups = [grouped[k] for k in ordered_keys]
        self.btn_start.setEnabled(True)
        self.btn_stop.setEnabled(False)
        if not self.nv_search_candidates:
            return
        self._show_naver_search_dialog()

    def _show_naver_search_dialog(self):
        if self.nv_search_dialog is None:
            dlg = QDialog(self)
            dlg.setModal(True)
            dlg.setWindowTitle("네이버 검색 결과 선택")
            dlg.resize(1300, 620)
            lay = QVBoxLayout(dlg)

            table = QTableWidget(0, 8)
            table.setHorizontalHeaderLabels(["선택", "건수", "대표매물번호", "지역", "종류", "거래", "매물명(그룹)", "등록일"])
            ...
            btn_run = QPushButton("선택 매물 수집 시작")
            ...
            btn_all.clicked.connect(lambda: self._set_naver_search_checks(Qt.CheckState.Checked))
            btn_none.clicked.connect(lambda: self._set_naver_search_checks(Qt.CheckState.Unchecked))
            btn_run.clicked.connect(self._start_naver_selected_crawl)
            btn_close.clicked.connect(dlg.reject)
            table.cellClicked.connect(self._on_naver_search_cell_clicked)

            self.nv_search_dialog = dlg
            self.nv_search_table = table

        if self.nv_search_table is None:
            return
        self.nv_search_table.setRowCount(0)
        for group in self.nv_search_groups:
            items = list(group.get("items", []))
            if not items:
                continue
            first = items[0]
            count = len(items)
            r = self.nv_search_table.rowCount()
            self.nv_search_table.insertRow(r)

            chk = QTableWidgetItem("")
            chk.setFlags(Qt.ItemFlag.ItemIsEnabled)
            chk.setCheckState(Qt.CheckState.Checked)
            # setCheckState 가 UserCheckable 을 켜면 체크 열 클릭이 Qt·핸들러에서 이중 토글됨 → 행 아무 곳이나 한 번만 토글되게 끔
            chk.setFlags(chk.flags() & ~Qt.ItemFlag.ItemIsUserCheckable)
            chk.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
            self.nv_search_table.setItem(r, 0, chk)
            self.nv_search_table.setItem(r, 1, QTableWidgetItem(str(count)))
            self.nv_search_table.setItem(r, 2, QTableWidgetItem(str(first.get("article_no", ""))))
            self.nv_search_table.setItem(r, 3, QTableWidgetItem(str(first.get("region_name", ""))))
            self.nv_search_table.setItem(r, 4, QTableWidgetItem(str(first.get("realestate_type_name", ""))))
            self.nv_search_table.setItem(r, 5, QTableWidgetItem(str(first.get("trade_type_name", ""))))
            group_name = str(group.get("name", ""))
            if count > 1:
                group_name = f"{group_name} (+{count - 1}건)"
            name_item = QTableWidgetItem(group_name)
            name_item.setToolTip(str(group.get("name", "")))
            self.nv_search_table.setItem(r, 6, name_item)
            self.nv_search_table.setItem(r, 7, QTableWidgetItem(str(first.get("confirm_date", ""))))

        self.nv_search_dialog.exec()

    def _set_naver_search_checks(self, state: Qt.CheckState):
        if self.nv_search_table is None:
            return
        for row_idx in range(self.nv_search_table.rowCount()):
            item = self.nv_search_table.item(row_idx, 0)
            if item is not None:
                item.setCheckState(state)

    def _on_naver_search_cell_clicked(self, row: int, col: int):
        if self.nv_search_table is None:
            return
        item = self.nv_search_table.item(row, 0)
        if item is None:
            return
        item.setCheckState(
    ...
    def _start_naver_selected_crawl(self):
        if self.nv_search_table is None:
            return
        selected: List[Dict[str, Any]] = []
        selected_ids: set[str] = set()
        for row_idx in range(self.nv_search_table.rowCount()):
            chk = self.nv_search_table.item(row_idx, 0)
            if chk is None or chk.checkState() != Qt.CheckState.Checked:
                continue
            if row_idx >= len(self.nv_search_groups):
                continue
            items = self.nv_search_groups[row_idx].get("items", [])
            for item in items:
                article_no = str((item or {}).get("article_no", "")).strip()
                if article_no and article_no in selected_ids:
                    continue
                if article_no:
                    selected_ids.add(article_no)
                selected.append(item)

        if not selected:
            QMessageBox.warning(self, "오류", "수집할 매물을 하나 이상 체크하세요.")
            return
        if self.worker is not None:
            QMessageBox.information(self, "안내", "작업이 아직 실행 중입니다. 완료 후 다시 시도하세요.")
            return

        if self.nv_search_dialog is not None:
            self.nv_search_dialog.accept()

        params = dict(self.nv_last_params or {})
        params["mode"] = "crawl_selected"
        params["selected_articles"] = selected
        params["timestamp"] = datetime.now().strftime("%y%m%d_%H%M%S")
        params["region_for_file"] = f"{params.get('region_for_file', '선택')}_선택"

        self._prepare_worker_start()
        self.btn_start.setEnabled(False)
        self.btn_stop.setEnabled(True)
        self.nv_worker_mode = "crawl_selected"
        self.progress_bar.setValue(0)
        self.progress_text.setText("네이버 선택 매물 수집 중")
        self.progress_count.setText(f"0/{len(selected)}")
        self.worker_thread = QThread(self)
        self.worker = NaverWorker(params)
        self.worker.moveToThread(self.worker_thread)
        self.worker_thread.started.connect(self.worker.run)
        ...
        self.worker_thread.finished.connect(self._cleanup_worker)
        self.worker_thread.start()
        self._show_log_dialog(non_modal=True)

    def _show_naver_searching_dialog(self, message: str):
        if self.nv_searching_dialog is None:
            dlg = QDialog(self)
            dlg.setWindowTitle("네이버 검색 진행 중")
            dlg.setWindowModality(Qt.WindowModality.ApplicationModal)
            dlg.setModal(True)
            dlg.setMinimumSize(520, 220)
            lay = QVBoxLayout(dlg)
            lay.setContentsMargins(20, 20, 20, 20)
            lay.setSpacing(14)

            title = QLabel("네이버 검색 중")
            title.setStyleSheet("font-size:20px; font-weight:800; color:#5b21b6;")
            lay.addWidget(title)

            status = QLabel(message)
            status.setWordWrap(True)
            status.setStyleSheet("font-size:14px; font-weight:600; color:#1f2937;")
            lay.addWidget(status)

            note = QLabel("필요하면 아래 버튼으로 즉시 검색을 중단할 수 있습니다.")
            note.setStyleSheet("font-size:13px; color:#6b7280;")
            lay.addWidget(note)

            btn_row = QHBoxLayout()
            btn_row.addStretch(1)
            btn_stop = QPushButton("검색 중단")
            btn_stop.setMinimumHeight(42)
            btn_stop.setStyleSheet(
                "QPushButton{background:#7c3aed; color:white; font-size:15px; font-weight:800; border:none; border-radius:10px; padding:0 20px;}"
                "QPushButton:hover{background:#6d28d9;}"
                "QPushButton:pressed{background:#5b21b6;}"
            )
            btn_stop.clicked.connect(self._cancel_naver_search_from_modal)
            btn_row.addWidget(btn_stop)
            lay.addLayout(btn_row)

            self.nv_searching_dialog = dlg
            self.nv_searching_status_label = status

        if self.nv_searching_status_label is not None:
            self.nv_searching_status_label.setText(str(message or "").strip())
        self.nv_searching_dialog.show()
        self.nv_searching_dialog.raise_()
        self.nv_searching_dialog.activateWindow()

    def _close_naver_searching_dialog(self):
        if self.nv_searching_dialog is not None:
            self.nv_searching_dialog.hide()

    def _cancel_naver_search_from_modal(self):
        if self.worker and self.nv_worker_mode == "search":
            self.worker.cancel()
            if self.nv_searching_status_label is not None:
                self.nv_searching_status_label.setText("중단 요청됨... 응답을 기다리는 중입니다.")
            self.progress_text.setText("네이버 검색 중단 요청됨...")

    def _on_stop(self):
        if self.worker:
            self.worker.cancel()
            self.progress_text.setText("중단 요청됨...")
            if self.nv_worker_mode == "search" and self.nv_searching_status_label is not None:
                self.nv_searching_status_label.setText("중단 요청됨... 응답을 기다리는 중입니다.")
    ...
    def _on_naver_status(self, msg: str):
        ...
        if self.nv_worker_mode == "search" and self.nv_searching_status_label is not None:
            self.nv_searching_status_label.setText(text)
    ...
    def item_to_display_row(row: Any) -> tuple:
        ...
        if any(k in r for k in ("건물명", "전체주소", "주소_호실", "물건번호")):
            ...
            floor = str(r.get("해당층") or r.get("층") or "-").strip()
            hosu = str(r.get("호수") or r.get("호") or "-").strip()
            return (no or "-", region or "-", kind or "-", trade or "-", name or "-", floor or "-", hosu or "-")
        ...
        floor = (
            _pick("해당층", "층")
            or _pick_by_key_contains("해당", "층")
            or "-"
        )
        hosu = (
            _pick("호수", "호")
            or _pick_by_key_contains("호수")
            or "-"
        )
        return (no, region, kind, trade, name, floor, hosu)
    ...
    def _show_log_dialog(self, non_modal: bool = False):
        if self.log_dialog is None:
            ...
            table.setHorizontalHeaderLabels(["매물번호", "매물지역", "부동산종류", "거래방식", "매물명", "층수", "호수"])
            table.setColumnWidth(0, 110)
            table.setColumnWidth(1, 220)
            table.setColumnWidth(2, 110)
            table.setColumnWidth(3, 130)
            table.setColumnWidth(4, 380)
    ...
    def _format_log_row_for_display(row: Iterable[str]) -> tuple:
        ...
        return tuple(vals[i] if i < len(vals) else "-" for i in range(7))
