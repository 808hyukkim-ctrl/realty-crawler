        ...
        try:
            ...
            if mode == "crawl_selected":
                ...
                for i, article in enumerate(selected_articles, 1):
                    ...
                    try:
                        row = crawler.extract_detail_v2(article_no, is_hosu_needed=False)
                    except Exception:
                        row = None
                    if not row:
                        continue
...
class MainWindow(QMainWindow, ScheduleMixin):
    ...
    def _setup_ui(self):
        ...
        footer = QLabel(
            f'copyright reserved {current_year} codecoon | '
            f'<a href="{PRODUCT_URL}">{PRODUCT_URL}</a>'
        )
    ...
    def _on_naver_search_ready(self, candidates: List[Dict[str, Any]]):
        ...
        for row in self.nv_search_candidates:
            name = str((row or {}).get("article_full_name", "") or (row or {}).get("article_name", "")).strip()
